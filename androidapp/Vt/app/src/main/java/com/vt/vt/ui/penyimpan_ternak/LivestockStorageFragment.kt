...
import com.vt.vt.databinding.FragmentLivestockStorageBinding
import com.vt.vt.ui.penyimpan_ternak.adapter.LivestockStorageAdapter
import dagger.hilt.android.AndroidEntryPoint

@AndroidEntryPoint
class LivestockStorageFragment : Fragment(), Toolbar.OnMenuItemClickListener {

    private var _binding: FragmentLivestockStorageBinding? = null
    private val binding get() = _binding!!

    private val livestockStorage: LivestockStorageViewModel by viewModels()

    override fun onCreateView(
        inflater: LayoutInflater, container: ViewGroup?,
        savedInstanceState: Bundle?
    ): View {
        _binding = FragmentLivestockStorageBinding.inflate(inflater, container, false)
        return binding.root
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        with(binding) {
            this.appBarPenyimpananTernak.topAppBar.apply {
                ...
                setOnMenuItemClickListener(this@LivestockStorageFragment)
            }
        }
        observerView()
    }
    ...
    private fun setRecyclerView(
        ...
        val livestockStorageAdapter =
            LivestockStorageAdapter(requireActivity(), livestockStorage)
        if (data.isNotEmpty()) {
    ...
    override fun onMenuItemClick(item: MenuItem?): Boolean {
        when (item?.itemId) {
            R.id.action_add_area -> {
                LivestockStorageAdapter.isUpdate = false
