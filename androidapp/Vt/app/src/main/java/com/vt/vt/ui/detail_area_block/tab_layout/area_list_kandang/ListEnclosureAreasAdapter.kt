...
class ListEnclosureAreasAdapter(
    private val context: Context, private val viewModel: DataKandangViewModel
) : ListAdapter<Sleds, ListEnclosureAreasAdapter.ViewHolder>(
    DIFF_CALLBACK
) {
    ...
    inner class ViewHolder(private val binding: ItemDetailAreaBlockBinding) :
        ...
        fun bindTo(sleds: Sleds) {
            with(binding) {
                val sledName = sleds.name.substringAfter(" ")
                tvDateItemDetailAreaBlock.text = sleds.createdAt
                tvTitleAnimalCage.text = sledName
